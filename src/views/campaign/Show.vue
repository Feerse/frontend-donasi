				...
				<div class="bg-gray-200 p-3 rounded shadow-md mb-3">
					<div class="grid grid-cols-10 gap-4">
						<div class="col-span-2">
							<img
								:src="user.avatar"
								class="w-15 h-15 rounded-full shadow"
							/>
						</div>
					...
					<div class="grid grid-cols-10 gap-4">
						...
						<div class="col-span-9 mt-1">
							...
							<div class="text-sm mt-2 text-gray-500">
								...
								<span class="font-bold"
									>Rp.
									{{ formatPrice(donation.amount) }}</span
								>
							</div>
						</div>
					</div>
...
<script setup>
	...
	onMounted(() => {
		campaignStore.getDetailCampaign(route.params.slug);
	});

	const campaign = computed(() => campaignStore.campaign);
	const user = computed(() => campaignStore.user);
	const sumDonation = computed(() => campaignStore.sumDonation);
	const donations = computed(() => campaignStore.donations);
</script>
